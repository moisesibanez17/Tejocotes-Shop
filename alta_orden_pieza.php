...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_id && $result_id->num_rows > 0) {
        ...
        if ($result_order_id && $result_order_id->num_rows > 0) {
            ...
            $cantidad_restar = $_POST['cantidad_pieza'];

            // Resto del código para verificar el stock y realizar el alta en la orden de computadora
            $consulta_stock = $mysqli->query("SELECT Stock FROM pieza WHERE ID = $id_pieza");
            ...
            if ($consulta_stock && $consulta_stock->num_rows > 0) {
                ...
                if ($nuevo_stock >= 0) {
                    $actualizar_stock = $mysqli->query("UPDATE pieza SET Stock = $nuevo_stock WHERE ID = $id_pieza");

                    if ($actualizar_stock) {
                        $query = "INSERT INTO orden_pieza (id_orden, id_pieza, cantidad) " .
                            "VALUES ('$id_orden', '$id_pieza', '$cantidad_restar')";
                        ...
                        ";
                            ...
                            echo "<br/><input type=\"button\" id=\"botones\"value=\"Volver\" onclick=\"location.href='catalogo_computadoras.php'\">";
                        } else {
